...
int	check_cone_vals(char **splited, int arg_count)
{
	...
	{
		ft_putstr_fd("Error\nInvalid cone position: expected format 'x,y,z' with valid numeric coordinates.\n", 2);
		return (0);
	}
	if (!check_normal(splited[2]))
	{
		ft_putstr_fd("Error\nInvalid cone axis direction: must be a normalized vector 'x,y,z' with each component in [-1,1].\n", 2);
		return (0);
	}
	if (!check_float(splited[3]))
	{
		ft_putstr_fd("Error\nInvalid cone height: expected a positive floating-point number.\n", 2);
		return (0);
	}
	if (!check_float(splited[4]))
	{
		ft_putstr_fd("Error\nInvalid cone angle: expected a positive floating-point number (in degrees).\n", 2);
		return (0);
	}
	if (!check_color(splited[5]))
	{
		ft_putstr_fd("Error\nInvalid cone color: expected format 'r,g,b' with values between 0 and 255.\n", 2);
		return (0);
	}
	if (arg_count == 7)
	{
...
int	check_cone(char *line)
{
	...
	{
		ft_putstr_fd("Error\nInvalid cone format: expected 'co pos_x,y,z axis_x,y,z height angle r,g,b [texture_path]'\n", 2);
